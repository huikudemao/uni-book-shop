<template>
	<view class="cart">
		...
		<view class="tabbar">
			...
			<view class="all">
					<u-checkbox shape="circle" v-model="isAllChecked"></u-checkbox>全选
			</view>
	...
	export default {
		...
		computed: {
				...
				isAllChecked:{
					// list列表--->全选
					get(){
						// every() 方法测试数组的所有元素是否都通过了指定函数的测试。即检测isChecked是否都为true
						// 列表中是否都选中了
					...
					set(val){
						console.log(val);
						// forEach() 方法用于调用数组的每个元素，并将元素传递给回调函数。让isChecked都变成val值（true,false）
