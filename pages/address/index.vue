<template>
	<view>
		...
		<view @click="toOrder(res.id)" v-else class="item" v-for="(res, index) in siteList" :key="res.id">
			<view class="top">
			...
			<view class="bottom">
				...
				<view @click.stop="modify(res.id)"><u-icon  name="trash" :size="40" color="#999999"></u-icon></view>
			</view>
		</view>
...
export default {
	...
	methods: {
		...
		async modify(id){
			console.log("1111")
			console.log(id)
			await this.$u.api.deleteAddress(id)
			console.log("1111")
			uni.showToast({
				title:'删除成功',
				icon:'none'
